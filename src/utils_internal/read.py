...
for index, row in tqdm(df_test_batch.iterrows(), total=df_test_batch.shape[0]):
    ...
    while target_index > 0:
        ...
        is_special = False
        if tokenizer.eos_token and tokenizer.eos_token in tok_str:
            is_special = True
        if tokenizer.pad_token and tokenizer.pad_token in tok_str:
            is_special = True
            
        if any(char.isalnum() for char in tok_str) and not is_special:
