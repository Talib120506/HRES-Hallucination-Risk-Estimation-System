...
Two detection pipelines:
  - Whitebox (HRES): TinyLlama hidden-state extraction -> PCA -> SVM/XGBoost
...
PDFS_DIR       = os.path.join(BASE_DIR, "10 pdfs")
SVM_PATH       = os.path.join(BASE_DIR, "svm_model_final.pkl")
XGB_PATH       = os.path.join(BASE_DIR, "xgb_model_final.pkl")
SCALER_PATH    = os.path.join(BASE_DIR, "scaler_final.pkl")
PCA_PATH       = os.path.join(BASE_DIR, "pca_final.pkl")
VT_PATH        = os.path.join(BASE_DIR, "variance_threshold_final.pkl")
...
def get_classifiers():
    global _svm, _xgb, _scaler, _pca, _vt
    if _scaler is None:
        _scaler = joblib.load(SCALER_PATH)
        _pca = joblib.load(PCA_PATH)
        _vt = joblib.load(VT_PATH)
        pca_dim = _pca.n_components_
        if os.path.exists(SVM_PATH):
            svm_candidate = joblib.load(SVM_PATH)
            if svm_candidate.shape_fit_[1] == pca_dim:
                _svm = svm_candidate
        if os.path.exists(XGB_PATH):
            xgb_candidate = joblib.load(XGB_PATH)
            if xgb_candidate.n_features_in_ == pca_dim:
                _xgb = xgb_candidate
    return _svm, _xgb, _scaler, _pca, _vt
...
def whitebox_predict(pdf_path, question, answer):
    ...
    context = " ".join([p["text"] for p in pages[:3]])[:MAX_CTX_CHARS]
    ...
    svm, xgb, scaler, pca, vt = get_classifiers()
    ...
    if xgb is not None:
        xgb_pred = xgb.predict(features_pca)[0]
        xgb_proba = xgb.predict_proba(features_pca)[0]
        results["XGBoost"] = {
...
def blackbox_predict(pdf_path, question, answer):
    ...
    vecs = embedder.encode(all_chunks, batch_size=64, show_progress_bar=False,
...
def predict(pdf_file, question, answer, progress=gr.Progress()):
    ...
    if wb_err:
        ...
        for model_name, res in wb_results.items():
...
<div class="model-result-card">
  ...
  <div class="source-chunk">
    ...
    if wb_results:
        primary = wb_results.get("SVM", wb_results.get("XGBoost", {}))
        wb_label = primary.get("label")
        wb_conf = primary.get("confidence", 0)
    ...
    if wb_label and bb_verdict:
        ...
        if wb_says_hall and bb_says_hall:
            ...
            verdict_desc = "Whitebox flagged hallucination, but NLI does not contradict. Manual review recommended."
...
<div class="summary-table">
  ...
  <div class="summary-row">
    ...
    <div class="summary-cell cell-method">TinyLlama hidden states + SVM</div>
...
def predict_with_options(pdf_upload, preloaded_pdf, question, answer,
                         progress=gr.Progress()):
    """Wrapper that picks between uploaded PDF and preloaded PDF."""
    try:
        if pdf_upload is not None:
            pdf_path = pdf_upload.name if hasattr(pdf_upload, 'name') else pdf_upload
        elif preloaded_pdf and preloaded_pdf != "None":
            pdf_path = os.path.join(PDFS_DIR, preloaded_pdf)
        else:
            err = _html_error("Please upload a PDF or select a preloaded one.")
            return err, "", ""
...
def build_ui():
    ...
    preloaded_choices = ["None"] + preloaded
        ...
        with gr.Row(equal_height=False):
            ...
            with gr.Column(scale=1, min_width=340):
                ...
                preloaded_dd = gr.Dropdown(
                    ...
                    value="None",
                    label="Or select a preloaded manual",
                )
...
<div style="font-size:0.78em;color:#94a3b8;margin-top:-6px;margin-bottom:10px">
  TinyLlama hidden states &#8594; PCA &#8594; SVM / XGBoost
</div>""")
                        wb_out = gr.HTML(
...
<table class="how-table">
  ...
  <tr>
    ...
    <td>Feed PDF context + Q&A into TinyLlama</td>
    <td>Extract and chunk all PDF pages</td>
  </tr>
  <tr>
    <td><span class="step-num">2</span> Encode</td>
    <td>Extract 2048-D hidden-state vector (last layer)</td>
    <td>Embed chunks with MiniLM-L6-v2, build FAISS index</td>
  </tr>
  <tr>
...
<div class="app-footer">
  ...
  &middot; Dual-pipeline Whitebox + Blackbox Analysis
        ...
        run_btn.click(
            ...
            inputs=[pdf_upload, preloaded_dd, question, answer],
